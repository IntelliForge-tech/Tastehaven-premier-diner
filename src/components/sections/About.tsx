import { SectionTitle } from "@/components/common/SectionTitle";
import { gallery1 } from "@/data/gallery";
import { useAbout } from "@/hooks/useAbout";
import { DEFAULT_FEATURES } from "@/services/about.service";

/**
 * About section — Phase 12B.
 *
 * Content is now driven by the about_settings Supabase singleton row via
 * useAbout(). Falls back to the original hardcoded values (matching the
 * pre-CMS copy exactly) when the row is loading, missing, or has null
 * fields — the UI never crashes on undefined data.
 *
 * Styling, layout, animations (reveal, hover-lift, card-glass) and
 * responsive behaviour are completely unchanged from the pre-CMS version.
 *
 * If `isVisible` is false the entire section is omitted from the DOM
 * while the rest of the homepage continues to function normally.
 */
export function About() {
  const { about, isLoading } = useAbout();

  // ── Resolve content: DB row → hardcoded fallbacks ─────────────────────
  const sectionTitle = about?.sectionTitle ?? "Our Story";
  const heading = about?.heading ?? "A haven for the curious palate.";
  const description =
    about?.description ??
    "Born from a love of the neighborhood market, Taste Haven brings together seasonal ingredients, global technique, and a room designed for slow, unhurried evenings. Every plate is a small ceremony.";

  // Features: DB row → service defaults → constants fallback.
  const rawFeatures = about?.features;
  const features =
    rawFeatures && rawFeatures.length > 0
      ? rawFeatures
      : DEFAULT_FEATURES;

  const imageUrl = about?.imageUrl ?? null;
  const badgeYear = about?.badgeYear ?? "2012";
  const badgeText = about?.badgeText ?? "A decade of memorable evenings.";
  const showBadge = badgeYear || badgeText;

  // While loading, render with defaults so there's no layout shift.
  // Once loaded, hide entirely if admin has toggled visibility off.
  if (!isLoading && about && !about.isVisible) {
    return null;
  }

  // Resolve image source: DB URL → imported fallback asset.
  const imgSrc = imageUrl ?? gallery1;

  return (
    <section id="about" className="relative py-24 md:py-32">
      <div className="mx-auto grid max-w-7xl gap-14 px-5 md:px-8 lg:grid-cols-2 lg:items-center">

        {/* ── Left column — text & feature cards ───────────────────────── */}
        <div className="reveal">
          <SectionTitle>{sectionTitle}</SectionTitle>
          <h2 className="mt-3 font-display text-4xl md:text-5xl">{heading}</h2>
          {description && (
            <p className="mt-5 text-muted-foreground">{description}</p>
          )}

          {features.length > 0 && (
            <div className="mt-8 grid grid-cols-2 gap-4">
              {features.map((f, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-border bg-card/50 p-5 hover-lift"
                >
                  <div className="grid h-10 w-10 place-items-center rounded-full bg-primary/15 text-primary">
                    <i className={`fa-solid fa-${f.icon}`} aria-hidden="true" />
                  </div>
                  <h3 className="mt-3 font-display text-lg">{f.title}</h3>
                  {f.description && (
                    <p className="mt-1 text-sm text-muted-foreground">{f.description}</p>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>

        {/* ── Right column — image & badge ─────────────────────────────── */}
        <div className="relative reveal">
          <div className="relative aspect-[4/5] overflow-hidden rounded-2xl">
            <img
              src={imgSrc}
              alt="Restaurant interior"
              loading="lazy"
              width={900}
              height={900}
              className="h-full w-full object-cover"
            />
          </div>

          {showBadge && (
            <div className="absolute -bottom-8 -left-6 hidden w-56 rounded-xl card-glass p-5 shadow-card md:block">
              <div className="text-xs uppercase tracking-widest text-muted-foreground">Since</div>
              {badgeYear && (
                <div className="font-display text-4xl text-gradient-gold">{badgeYear}</div>
              )}
              {badgeText && (
                <div className="mt-1 text-sm text-muted-foreground">{badgeText}</div>
              )}
            </div>
          )}
        </div>

      </div>
    </section>
  );
}
